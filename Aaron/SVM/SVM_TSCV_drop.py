...
# Full data
train_data_path = os.getcwd() + f"/../data/TSCV/train_data_tscv"
val_data_path = os.getcwd() + f"/../data/TSCV/val_data_tscv"

# 33%
# train_data_path = os.getcwd() + f"/../data/33%/train_data_tscv"
# val_data_path = os.getcwd() + f"/../data/33%/val_data_tscv"

# 66%
# train_data_path = os.getcwd() + f"/../data/66%/train_data_tscv"
# val_data_path = os.getcwd() + f"/../data/66%/val_data_tscv"
...
for i in range(4):
    X_train[i] = pd.read_csv(train_data_path + f'_{i}_f.csv')
    X_val[i] = pd.read_csv(val_data_path + f'_{i}_f.csv')
...
with open(f'drop{drop_count}.txt', 'r') as f:
    for line in f:
        line = line.strip()
        for i in range(4):
            X_train[i] = X_train[i].drop(line, axis=1)
            X_val[i] = X_val[i].drop(line, axis = 1)
        X_whole = X_whole.drop(line, axis=1)
        X_test[stage] = X_test[stage].drop(line, axis=1)
...
columns_to_encode = ['home_pitcher', 'away_pitcher']
for i in range(4):
...
# f = open(f'accuracy_spreads/gaussian_acc_spread_S{stage + 1}_N{N}_{col}col_TSCV.txt', 'w')
...
predict.to_csv(f"predictions/RBF_C{C_opt}_G{gamma_opt}_TSCV_S{stage + 1}_-{drop_count}col.csv", index_label="id")
