...
We use the following heuristic to calculate the final cross validation errors: $Acc_{TSCV}=$1*Acc_1+2*Acc_2+3*Acc_3+4*Acc_$/10$ where $Acc_i$ are calculated with validation $i$, $i = 1, 2, 3, 4$
