...
def load_data(train_file, val_file):
    ...
    Removes all categorical columns and 'id' from both datasets.
    """
    try:
        ...
        train_data_clean = train_data.drop(columns=categorical_cols, errors='ignore')
        val_data_clean = val_data.drop(columns=categorical_cols, errors='ignore')
        
        required_cols = ['home_team_win', 'id']
        ...
        X_train = train_data_clean.drop(['home_team_win', 'id'], axis=1)
        y_train = train_data_clean['home_team_win']
        
        X_val = val_data_clean.drop(['home_team_win', 'id'], axis=1)
...
def main():
    ...
    for params in param_combinations:
        ...
        with open("Results.txt", "a") as f:
            f.write(f"Parameters: {params}, Eval: {overall_E_val:.4f}, Ein: {overall_E_in:.4f}\n")
