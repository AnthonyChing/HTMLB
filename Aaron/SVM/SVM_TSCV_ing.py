...
for i in range(4):
    ...
    # Drop is_night_game NaN rows
    X_train[i] = X_train[i].dropna(subset=['is_night_game'])
    X_val[i] = X_val[i].dropna(subset=['is_night_game'])

    y_train[i] = X_train[i]['is_night_game']
    y_val[i] = X_val[i]['is_night_game']
    X_train[i] = X_train[i].drop('is_night_game', axis=1)
    X_val[i] = X_val[i].drop('is_night_game', axis=1)

# Test data
X_test_1 = pd.read_csv(os.getcwd() +  "/../data/Test_ing/stage_1_filled_ing.csv")
X_test_2 = pd.read_csv(os.getcwd() +  "/../data/Test_ing/stage_2_filled_ing.csv")
...
X_test[stage] = X_test[stage].dropna(subset=['is_night_game'])
...
# svm_save_model(f'models/RBF_C{C_opt}_G{gamma_opt}_TSCV_S{stage + 1}_{col}col_ing.model', m)


# %%
# Predict
# m = svm_load_model(f'models/RBF_C{C_opt}_G{gamma_opt}_TSCV_S{stage + 1}_{col}col_ing.model')
p_labs, p_acc, p_vals = svm_predict([], X_t, m)

tmp = [bool(x) for x in p_labs]

predict = pd.DataFrame()
predict['is_night_game'] = tmp
predict.to_csv(f"predictions/RBF_C{C_opt}_G{gamma_opt}_TSCV_S{stage + 1}_{col}col_ing.csv", index_label="id")
