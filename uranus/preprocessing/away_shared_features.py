...
df = pd.read_csv('home_team_combined.csv', dtype=str)
...
if not os.path.exists('away_teams'):
    os.makedirs('away_teams')

# Get all unique home_team_abbr values
unique_teams = df['away_team_abbr'].unique()

# For each unique team, filter rows and save to separate CSV
for team in unique_teams:
    team_df = df[df['away_team_abbr'] == team]
    team_df.to_csv(f'away_teams/{team}.csv', index=False)